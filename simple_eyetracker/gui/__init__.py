...
from simple_eyetracker.settings import settings as global_settings
...
class EyeTrackerGUI:
    ...
    def update_tracker_view(self):
        if (self.controller is None) or (self.controller.camera is None):
            return
        ...
        if time_since_last_update > time_between_updates:
            ...
            self.frame_rate = np.mean(np.array(self.frame_rates))
            self.frame_rates = []
